...
def trainingFunction(config:dict):
    ...
    model = RectifiedConditionalFlow(
    ...
    for epoch in range(numEpochs):
        ...
        for batchIdx, batch in enumerate(trainingDataloader):
            ...
            u = torch.rand(batchSize,device=device)
            t = 1 - (2 * u - 1).abs() 
            
            
            X0  = torch.randn_like(sliceI,device=device) #B,C,H,W
            X1 = sliceI
            target = X1 - X0
            
            tEqn = t.view(-1,1,1,1)
            
            Xt = (1-tEqn)*X0 + tEqn*X1
            
            inputConcat = torch.cat([slicePlusOne,Xt,sliceMinusOne],dim=1) #B,C,H,W
            
            outputVfield = model.forward(inputConcat,t)
            
            loss=F.mse_loss(outputVfield,target)
        ...
        with torch.no_grad():
            for batch in validationDataloader:
                ...
                outputVfield = emaModel.module.forward(inputConcat,t)
