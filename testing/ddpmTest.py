...
class DDPMSampler(nn.Module):
    """Wrapper for DDPM sampling during inference"""
    def __init__(self, checkpoint: str, cfg: DictConfig):
        super().__init__()
        
        self.model = ConditionalDDPM(cfg=cfg)
...
def chooseBestCheckpointValidation(cfg: DictConfig):
    ...
    for checkpoint in checkpoints:
        ...
        model = DDPMSampler(checkpoint, cfg=cfg)
        model.to(device)
...
def TestingFunction(cfg: DictConfig):
    ...
    model = DDPMSampler(checkpoint=bestCheckpoint,cfg=cfg)
    model.to(device)
    ...
    currentSolverStepDir = Path(imageSaveDir) / f"DDPM_{numSteps}"
